...
yum -y install gcc make gcc-c++ kernel-devel-`uname -r` zlib-devel openssl-devel readline-devel sqlite-devel perl wget dkms nfs-utils pcre-devel pcre net-tools bzip2 > /dev/null

#more packages
yum -y install man nc vim nano tmux htop tree lsof curl wget screen telnet mailx unzip yum-utils ccze rsync ncdu mosh pv rlwrap > /dev/null
...
yum -y install apachetop dstat sysstat iotop smartmontools > /dev/null

#Ag installation: https://github.com/ggreer/the_silver_searcher

yum -y install the_silver_searcher > /dev/null